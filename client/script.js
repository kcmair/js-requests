...
sayHelloButton.addEventListener('mouseover', sayHelloMsOv);
...
sayHelloButton.addEventListener('mouseout', sayHelloMsOut);
...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/{Squirrels}')
        .then(response => {
            document.getElementById('repeat-text').innerHTML = response
        })
        .catch (error => console.log(error))
};
...
const thisQuery = () => {
    axios.get ('http://localhost:3000/query-test/?name=Keith')
        .then(response => {
            console.log(response);
        })
        .catch (error => console.log(error))
};
document.getElementById('query-button').addEventListener('click', thisQuery);
...
const createFood = () => {
    ...
    axios.post('http://localhost:3000/food', body)
        .then(res => {
            foodInput = document.createElement('p')
            foodInput.textContent = res.data
            document.querySelector('body').appendChild(foodInput)
